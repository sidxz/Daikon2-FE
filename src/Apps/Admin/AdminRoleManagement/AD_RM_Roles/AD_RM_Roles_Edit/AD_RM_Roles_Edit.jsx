...
const AD_RM_Roles_Edit = () => {
  ...
  useEffect(() => {
    if (selectedRole === null || selectedRole?.id !== params?.id) {
      fetchRole(params.id);
    }
  }, [params.id, fetchRole, selectedRole]);
  ...
  useEffect(() => {
    if (selectedRole) {
      formik.setValues({
        ...
        selfAccessLevel: selectedRole.selfAccessLevel || 0,
        organizationAccessLevel: selectedRole.organizationAccessLevel || 0,
        allAccessLevel: selectedRole.allAccessLevel || 0,
  ...
  if (isFetchingRole || isFetchingUsers) {
...
export default observer(AD_RM_Roles_Edit);
