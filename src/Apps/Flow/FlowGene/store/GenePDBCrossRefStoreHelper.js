...
export function _helper_formatLigands(unformattedLigands) {
  let ligands = [];
  unformattedLigands[Object.keys(unformattedLigands)[0]].forEach((ligand) => {
    ligands.push(ligand.chem_comp_name);
  });

  let formattedLigands = arrToInstanceCountObj(ligands);

  return formattedLigands;
}
