...
const MLRegistrationStep2 = ({ inputs }) => {
  ...
  useEffect(() => {
    ...
    const sendDryRunRequest = async () => {
      if (!inputs || inputs.length === 0) {
        if (!cancelled) setDryRunResults([]); // fix
        return;
      }

      setLoading(true);
      try {
        const previewResponse = await MolDbAPI.registerMoleculePreview(inputs);
        ...
        if (!cancelled) {
          setDryRunResults(Array.isArray(results) ? results : [results]);
        }
      } catch (err) {
        console.error("Error during dry run request:", err);
        if (!cancelled) setDryRunResults([]);
      } finally {
  ...
  const previewStatusBody = (rowData) => {
  ...
  const stats = useMemo(() => {
    ...
    const returned = dryRunResults?.length ?? 0;
  ...
  const downloadPreviewResults = async () => {
    await exportToExcel({
      ...
      fileName: `MLRegistration_PreviewResults_${new Date()
        .toISOString()
        .slice(0, 10)}`,
      sheetName: "PreviewResults",
      includeId: true,
    ...
    return (
      <div className="w-full flex flex-column gap-2 bg-surface-50 border-round-lg">
        ...
        <div className="w-full flex justify-content-end gap-2 px-2 pb-2">
          <Button
            icon="pi pi-download"
            label="Download Preview Results"
            className="p-button-sm"
            disabled={loading || (dryRunResults?.length ?? 0) === 0}
...
export default observer(MLRegistrationStep2);
