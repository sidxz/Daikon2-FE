...
const PhenotypicValidatedHitsImporter = ({ screenId, existingHits }) => {
  ...
  let handleOnDrop = (r) => {
    ...
    r.data.forEach((hit) => {
      ...
      if (existingHitCompoundIds.includes(hit.Id)) {
        ...
        if (existingHit.iC50 !== hit.IC50 || existingHit.mic !== hit.MIC) {
          index = index + 1;
          hitsToUpdate.push({
            Id: existingHit.id,
            Index: index,
...
export default observer(PhenotypicValidatedHitsImporter);
